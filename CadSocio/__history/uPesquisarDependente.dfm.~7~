object frmPesquisaDeDependente: TfrmPesquisaDeDependente
  Left = 0
  Top = 0
  Caption = 'frmPesquisaDeDependente'
  ClientHeight = 435
  ClientWidth = 679
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 679
    Height = 33
    Align = alTop
    Color = clMenu
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object edtPesquisar: TEdit
      AlignWithMargins = True
      Left = 182
      Top = 4
      Width = 200
      Height = 25
      Align = alLeft
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      ExplicitHeight = 24
    end
    object btnPesquisar: TBitBtn
      AlignWithMargins = True
      Left = 388
      Top = 4
      Width = 28
      Height = 25
      Align = alLeft
      BiDiMode = bdLeftToRight
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFF}
      ParentBiDiMode = False
      ParentFont = False
      Spacing = 10
      TabOrder = 1
      OnClick = btnPesquisarClick
    end
    object cmbFiltros: TComboBox
      AlignWithMargins = True
      Left = 4
      Top = 4
      Width = 172
      Height = 22
      Align = alLeft
      Style = csOwnerDrawFixed
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      Items.Strings = (
        'Nome Socio'
        'Nome Dependente')
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 33
    Width = 679
    Height = 367
    Align = alClient
    DataSource = dsDependente
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ParentFont = False
    ReadOnly = True
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
  end
  object Panel2: TPanel
    Left = 0
    Top = 400
    Width = 679
    Height = 35
    Align = alBottom
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    object btnAlterar: TBitBtn
      AlignWithMargins = True
      Left = 575
      Top = 4
      Width = 100
      Height = 27
      Align = alRight
      Caption = 'Ver Detalhes'
      TabOrder = 0
    end
  end
  object aqrDependente: TADOQuery
    Active = True
    Connection = DM.conexao
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'SELECT NOME_SOCIO,CASE WHEN T1.STATUS = 1 THEN '#39'ATIVO'#39' ELSE '#39'DES' +
        'ATIVADO'#39' END AS SITUACAO,NOME_DEPENDENTE '
      'FROM SOCIO AS T1'
      'INNER JOIN DEPENDENTE  AS T2 ON  T1.ID_SOCIO = T2.ID_SOCIO')
    Left = 336
    Top = 224
    object aqrDependenteNOME_DEPENDENTE: TStringField
      DisplayLabel = 'NOME DO DEPENDENTE'
      FieldName = 'NOME_DEPENDENTE'
      Size = 40
    end
    object aqrDependenteNOME_SOCIO: TStringField
      DisplayLabel = 'NOME DO SOCIO'
      FieldName = 'NOME_SOCIO'
      Size = 40
    end
    object aqrDependenteSITUACAO: TStringField
      FieldName = 'SITUACAO'
      ReadOnly = True
      Size = 10
    end
  end
  object dsDependente: TDataSource
    DataSet = aqrDependente
    Left = 528
    Top = 160
  end
end
